var Riddha = {
    ...
    UI: {
        ...
        modal:
        {
            ...
            show: function (modalId, viewUrl, PreLoadedOrNot, callback) {

                debugger;
                Riddha.UI.modal.callback = callback;

                $("#" + modalId).modal("show");
                if (PreLoadedOrNot == false) {
                    $("#" + modalId + " div.modal-body").load(viewUrl);
                    Riddha.UI.modal.callback = callback;
                    //popup.show();
                }
...
ko.bindingHandlers.ckeditor = {
    init: function (element, valueAccessor) {
        ko.bindingHandlers.ckeditor.bind(element, valueAccessor());
    },
    update: function (element, valueAccessor) {

        ko.bindingHandlers.ckeditor.bind(element, valueAccessor);
        var editor = CKEDITOR.instances.element1;
        if (editor) { editor.destroy(true); }
        CKEDITOR.replace('element1');
        CKEDITOR.instances.element1.setData(valueAccessor()());
        CKEDITOR.instances.element1.on('blur', function () {
            var value = CKEDITOR.instances.element1.getData();
            valueAccessor()(value);
